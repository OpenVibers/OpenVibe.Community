'use strict';
// deploy/nginx/openvibe.community.conf: the shared navbar asks /auth/me on every page view, so the session probe has its own location on
...
const conf = fs.readFileSync(path.join(__dirname, '..', ...'deploy/nginx/openvibe.community.conf'.split('/')), 'utf8');
...
assert.match(probe, /limit_req zone=ovcommunity_api burst=30 nodelay;/);
assert.match(auth, /limit_req zone=ovcommunity_auth burst=10 nodelay;/);
